...
- [ ] **🔴 fagan — не чинить/не диагностировать инфру (Cleanroom-дисциплина).** claude-прогон: fagan вместо
  «прогнал гейт → GREEN accept / RED reject по пункту» ушёл в DevOps — диагностировал `docker pull EOF`,
  **тянул `golang:1.25-alpine`, ретраил, трекал «Defect 1 fixed»**. Сильная модель «услужливо» чинит вместо
  узкого гейта. Фикс: жёсткое «прогони гейт ОДИН раз; не чини/не ретрай docker-сеть; env-сбой → всплыть оператору».
- [ ] **🔴 scaffolder — инфра-smoke обязан РЕАЛЬНО пулить+гонять Docker; смена Dockerfile/образа после scaffold → ре-валидация.**
  Корень эпизода fagan: инфра (Docker-suite, базовый образ) — гейт **scaffolder** (`run-tests.sh` smoke). Она утекла
  к fagan потому, что либо scaffolder-smoke не по-настоящему прогнал pull (кэш старого образа прошёл), либо
  Dockerfiles бампнули на `1.25-alpine` уже ПОСЛЕ scaffold-green (никто не перевалидировал). Инфра-гейт = scaffolder, не приёмка.
- [ ] **🟠 инфра-vs-дефект.** `run-tests.sh exit≠0` смешивает env-сбой (docker/сеть/нет образа) и дефект кода.
  Различать → env-сбой ≠ reject кода (не гнать в @linger, не чинить).
