...
### Новая роль — фронтдор требований (БТ → BRD)
- [ ] **Роль «прожарка требований».** Точка входа конвейера **до** planner: пользователь загружает **БТ**
  (сырое бизнес-требование) → `izi` отдаёт этой роли → роль **прожаривает** требования (интервью по скиллу
  `requirements-intake`: закрывает пробелы, диапазоны/enum/format полей, open-questions) и готовит **полный
  BRD** с полнотой, достаточной для конвейера (data-dictionary, failure-map, контрактные предпосылки).
  Выход роли = вход для `wirth-*` пайплайна. **Мотив:** в run08 диапазоны полей выдумывались на дизайне,
  т.к. требования приходили неполными; фронтдор-роль гарантирует полноту BRD на входе, а не латает на Gate #1.
  Реализация — **потом** (сначала доделать текущую ветку/прогон). Связано с [L130/L131 untraced-knob],
  `requirements-intake §7`, TASK.md data-dictionary.
