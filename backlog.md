...
Статус (16-07, пересчёт): **тело переведено у ~30/33** — Фаза 1 фактически завершена. Остаётся:
- **RU-тело: `domain-modeling`** — Ф0-скелет (не дописан, role-wiring в Ф1–Ф3), перевести отдельно;
- **Фаза 3 — framing:** ✅ сделано (`http-io`/`tmux-test-runner`/`vertical-slices`/`implementation-ticket-writer`
  расшиты с `Qwen3.5/слабую модель` → tier-agnostic `weaker tiers`);
- **Фаза 3 — литералы:** остаточные RU-строки в `architecture`(4), `component-tests`(4), `observability`(2),
  ...
  `security` секции `## Безопасность` в plan.md) — решить по `I18N.md` (перевод только когда писатель И читатель EN);
