...
- [ ] **🔴 Toolchain-версия рассинхронилась → docker pull у fagan (эпизод «fagan чинит докер»).** Трасса
  ...
  **fagan (сильная модель) полез чинить/пулить/ретраить** вместо reject/surface. Фикс — 4 слоя, **версия-агностично
  ...
  - **Механический гейт `validate-toolchain-consistency` (профиль-параметризованный, НЕ роль/скилл).** Профиль
    (`target-profiles.json`) объявляет файлы-носители toolchain-версии (service/cli: `**/go.mod` `go`-директива +
    `**/*Dockerfile*` base-tag); валидатор требует **единой версии** во всех → СКОС = BLOCKER на Gate #1/DoD
    (ловит ДО fagan). Стек-нейтрально: другой стек = + профиль, 0 правок стадий.
