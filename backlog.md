...
- [ ] **🔴 Правило старта работы: pull транка → ветка по git-conventions → работа (git — ничей).** `@hughes`
  ...
  - **Терминальный git-шаг приёмки** (напр. `@git-hand`, cheap tier) — на green: `commit → push → PR`
    (+ для chore-полосы `gh run watch` до green/red). Ровно то, что оператор делает руками сейчас.
  - _DoD:_ харнес сам заводит ветку от свежего транка и доводит работу до PR без ручного git; демо на chore-задаче.
