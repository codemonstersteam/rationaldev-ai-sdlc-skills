...
- [ ] **🔴 Toolchain-версия рассинхронилась → docker pull у fagan (эпизод «fagan чинит докер»).** Трасса
  ...
  **fagan (сильная модель) полез чинить/пулить/ретраить** вместо reject/surface. Фикс — 4 слоя, **версия-агностично
  ...
  - **Роль/скилл-дисциплина — АГНОСТИЧНАЯ (без версий).** Скилл `service-scaffold`/`program-implementation`:
    «используй toolchain скаффолда как есть; если зависимость форсит бамп — бампни **единым фронтом** ВСЕ
    манифесты + Docker-базу, затем **ре-валидируй инфру**; никогда частичный/молчаливый бамп». Ни `go`, ни `1.24` в прозе.
  - **fagan-дисциплина (агностично).** «Прогони гейт ОДИН раз; RED → reject по названному пункту / STOP;
    **env-сбой (registry/сеть/pull образа) → surface оператору, НЕ чинить/не ретрай/не диагностировать инфру.**»
  - **infra-vs-defect в выходе `validate-dod --run`.** Различать env-сбой (docker pull/сеть) vs дефект кода
    (провал ассерта) → env-сбой ≠ reject кода (не в @linger, не чинить), а сигнал оператору. scaffolder остаётся
    инфра-гейтом initial-версии; пост-scaffold дрифт ловит `validate-toolchain-consistency` на Gate #1.
