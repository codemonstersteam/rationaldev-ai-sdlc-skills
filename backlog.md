...
### Live-прогон 17-07 (tempconv greenfield, opencode GLM/Qwen) — findings
Первый end-to-end прогон rework-харнеса (greenfield маленького сервиса до имплементации). Вскрыл 3 бага; izi-фиксы
(#0) уже в PR (роутер glm-5.2 + вопросы по одному). Осталось:
- [ ] **🔴 validate-plan: ловить пропущенную data-зависимость между модулями слайса.** Тикет-граф под-специфицирован:
  `ticket-05 (convert.go)` использует `type ConvertCommand`, определённый `ticket-04 (command.go)`, но
  `blocked_by` у 05 = `[01,02]` (нет ребра `05→04`). Стали параллельными сиблингами → hughes сделал 05 раньше 04 →
  чтобы скомпилировать, `convert.go` **сам** определил `ConvertCommand` → на очереди 04 конфликт типа → hughes завис.
  `validate-plan.mjs` проверяет цикл/scaffold-корень/module-after-component, но **НЕ** требует «модуль, использующий
  символ/тип другого модуля, обязан `blocked_by` его». **Фикс:** moduledesigner/ticketer кодируют inter-module
  data-зависимости в `blocked_by`; `validate-plan` их проверяет (символ используется → ребро есть).
- [ ] **🟠 Anti-loop guard: детект «субагент повторяет себя / нет прогресса».** Qwen3.6-27b (hughes) на конфликте
  типа ушёл в **петлю дословного повторения** одного абзаца reasoning, ничего не пишет, жжёт токены (19/19 шагов,
  стоит). Харнес не детектит «нет outputs + повтор». **Фикс:** guard в раннере/watchdog — превышен step-budget без
  outputs ИЛИ повтор reasoning → abort субагента → `@linger`/split (не бесконечный луп).
- [ ] **🟡 omo не полностью отключён** (косметика): claude-агенты `oracle/prometheus/…` в TUI-дропдауне; перезапись
  глобального конфіга не убрала плагин. Дизейблить плагин, не только `model` в конфиге. Не мешает прогону.
_Не баги (разобрано по логу):_ «go test виснет на дедлоке» — дедлока не было (hughes чинил реальный IEEE-754 баг,
модуль закрыт green); «izi QUEUED-столл» — вызван конфликтом оператор-прерывания vs авто-драйвера за одну сессию.
