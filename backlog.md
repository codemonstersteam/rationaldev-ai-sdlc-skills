...
- **deprecates:** `watch-izi-resume.sh` (bench-сайдкар) — оставить только на не-сетевой silent-idle, если плагин
  сетевое закроет.
- **verify:** инъекция 504 в прокси → izi turn переживает без ручного нуджа и без сайдкара.
- **discovered:** прогоны 07-07 — 504 роняет turn izi; tmux-нудж хрупок (QUEUED). Место фикса — транспорт/плагин, не TUI.
