...
### Live-прогоны 17-07 — findings (tempconv, opencode + claude)
Первые end-to-end прогоны (greenfield маленького сервиса). Вскрыли дыры на планировании и приёмке:
- [x] **🔴 validate-plan: data-зависимости типов между модулями.** ✅ Сделано (`validateTypeDependencies` +
  проза ticketer/moduledesigner). Тикетер выводил `blocked_by` из call-графа module-tree, но ТИП-зависимость
  между сиблингами терялась: `ticket-05 convert(cmd: ConvertCommand)` не зависел от `ticket-04 command`
  (owner типа) → параллельны → convert реализовался раньше → сам переопределил тип → hughes завис в петле.
  Механически: `New<Type>`-конструктор = владелец типа; потребитель типа обязан `blocked_by` владельца.
- [x] **🔴 Toolchain-версия рассинхронилась → docker pull у fagan (эпизод «fagan чинит докер»).** ✅ ВСЕ 4 слоя. Трасса
  (claude-прогон pinout-openapi, CLI): шаблон `template-go-cli` пинит **go 1.24 / golang:1.24-alpine** (docker
  green из коробки). Зависимость `kin-openapi` **форсит go 1.25** → роль подняла `go.mod`→1.25 и Dockerfiles→
  golang:1.25 (**оправданно, dep-forced, НЕ «модель взяла latest»**), НО **непоследовательно**:
  `component-tests/go.mod` остался 1.24 → СКОС; и смену никто не перевалидировал (scaffolder проверил 1.24).
  У fagan `validate-dod --run` → `docker pull golang:1.25` → транзиентный registry EOF (образ есть в кэше) →
  **fagan (сильная модель) полез чинить/пулить/ретраить** вместо reject/surface. Фикс — 4 слоя, **версия-агностично
  (ни `go`, ни версий в ролях/скиллах — версия это ДАННЫЕ шаблона+go.mod, диктует dep-граф):**
  - [x] **Механический гейт `validate-toolchain-consistency` (профиль-параметризованный, НЕ роль/скилл).** ✅ Сделано.
    Профиль (`target-profiles.json` `.toolchain[]`) объявляет источники версии (service/cli: `go.mod` `go`-директива +
    `*Dockerfile*` golang-база); валидатор требует **единой версии** (major.minor) во всех → СКОС = BLOCKER.
    Чистое ядро `validateToolchainConsistency`/`extractToolchainVersions` в `lib/validators.mjs` (+7 юнит-тестов);
    standalone `harness/validate-toolchain-consistency.mjs`; встроен шагом 2b в `validate-dod.mjs` (мех-гейт @fagan,
    ловит ДО семантики). Версия-агностично (версия — данные файлов+профиля). Стек-нейтрально: другой стек = + `toolchain[]`, 0 правок.
  - [x] **Роль/скилл-дисциплина — АГНОСТИЧНАЯ (без версий).** ✅ `program-implementation` MUST NOT: «toolchain
    скаффолда как есть; dep форсит бамп → **единым фронтом** ВСЕ манифесты + container base + **ре-валидация**;
    никогда частичный/молчаливый». Ни `go`, ни `1.24` в прозе (термины toolchain/manifest/container base).
  - [x] **fagan-дисциплина (агностично).** ✅ `fagan.md` Move 1 шаг 2: «гейт ОДИН раз; code/DoD-RED → reject
    по пункту; **env-сбой (`validate-dod` метит `ENV`) → surface оператору, НЕ диагностировать/пулить/ретраить/чинить инфру**».
  - [x] **infra-vs-defect в выходе `validate-dod --run`.** ✅ `classifyRunFailure` (`lib/validators.mjs`, +3 теста):
    маркеры docker/registry/сети → `env` (оператору, НЕ @linger) vs провал ассерта → `defect`; шаг 4 метит причину.
    scaffolder остаётся инфра-гейтом initial-версии; пост-scaffold дрифт ловит `validate-toolchain-consistency`.
- [x] **🟠 anti-loop guard.** ✅ Сделано (opencode). Qwen-hughes на конфликте типа ушёл в петлю дословного повтора
  без outputs. Детект по СИГНАТУРАМ tool-call'ов: `detectLoop`/`toolCallSignature` (`enforcement/shared.mjs`, чисто,
  +11 тестов) — K подряд одинаковых ИЛИ короткий цикл (read↔edit) периода 2–3 повторён R раз → петля; прогресс
  (иной вызов) рвёт паттерн. Плагин `rational-guardrail` (ring-buffer по сессии, все tool кроме `task`): на детекте
  бросает → турн падает как **dropout** → izi эскалирует по genchi-genbutsu (@linger/split, T04). Пороги в
  `rational.config.json` (`loopEnabled/RunThreshold/CycleRepeats/Window`, из watchdog-блока models.config.json).
  guardrail-smoke 30/30. _Follow-up: claude-паритет (stateless-хуки → file-based ring-buffer; `detectLoop` готов к реюзу)._
- [x] **omo-изоляция — ПРОЕКТНО-ЛОКАЛЬНЫЙ конфиг вместо мутации global.** Разбор (18-07): omo грузится
  ТОЛЬКО через `plugin:["oh-my-openagent"]` в конфиге opencode (пруф: log 30.06–01.07 `agent="Sisyphus" model=claude-opus`
  при omo-эпохе; после — следов нет). Проектный конфиг **НЕ** отключает плагин из global — opencode мержит
  массивы плагинов, disable-механизма нет (подтверждено докой opencode). Старый `prepare-rational-sandbox.sh`
  **мутировал GLOBAL** (`~/.config/opencode/opencode.jsonc`, backup+restore) — хрупко: `.bak` нёс omo-plugin, а
  инструкция «вернуть omo» его реглобаливала. Фикс: скрипт пишет **`$SB/opencode.jsonc`** (модель+прокси, проектно),
  global **не трогает**, + гард предупреждает, если global всё ещё объявляет `plugin`. omo включается ТОЛЬКО
  проектно (образец `omo-sdlc-test/opencode.json`: `plugin:["oh-my-openagent"]` + `.opencode/oh-my-openagent.json`;
  omo сам резолвит `<pwd>/.opencode/oh-my-openagent.jsonc`).
  - [ ] **env-миграция (руки оператора, вне репо):** убрать `plugin`+`oh-my-openagent.json` из `~/.config/opencode`,
    перенести omo в его проект(ы) локально, привести global к нейтральному. Скрипт этого НЕ делает (не мутирует global).
_izi-фиксы (роутер glm-5.2 + вопросы по одному) — в PR #52. Не баги: «go test дедлок» (реальный IEEE-754 фикс), «QUEUED-столл» (конфликт оператор vs авто-драйвер)._

### Прогон 18-07 — chore-полоса + правило старта работы (git) (claude, pinout-openapi)
Трасса: оператор попросил «добавить CI на ПР в транк». Задача — репо-инфра (один `.github/workflows/ci.yml`),
...
- [ ] **🟠 Полоса `chore` — экономичный проход простой задачи ПО ПЛАНУ (третий вес).** У конвейера два веса:
  ...
  **нет класса chore/infra**, задачу впихивают в `rework-refactor`/`greenfield trivial` (неточно); (2) даже
  `trivial → @hughes` тянет `@gilb`→`@wirth-triage`→`@hughes`→layout-gate→`@fagan` (`validate-dod --run`:
  build/test/`run-tests`/README) — для конфиг-файла половина DoD N/A; (3) trivial прыгает в `@hughes` **без плана
  и без Gate #1** — а нужен *дешёвый план + гейт*, не *ноль плана*. Фикс (в идиоме профилей, izi остаётся тупым):
  - **`wirth-triage`: ось 0 (chore vs code)** перед остальными — «не трогает секрет модуля / контракт, не
    добавляет поведение, проверяемое компонентным тестом → `route=chore`»; пишет `mode=chore`. Судит триаж, не izi.
  - **`izi`: +1 строка маршрута + секция CHORE lane** — `@gilb` (1 проход, `agent-ready`) → `@wirth-planner`
    (мини `CHORE-PLAN.md`: файлы + **команда верификации** + rollback, без FRD/slices/spec/tickets) → **мини
    Gate #1** (оператор акцептит одностраничник) → `@hughes` под `mode=chore` (без io-скиллов) → приёмка =
    **запуск команды верификации**, не Go-DoD.
  - **`target-profiles.json`: профиль `chore`** — DoD «команда верификации зелёная», нет scaffold-шаблона,
    нет компонентных тестов; стадии делегируют профилю, не пишут `if chore` (open-closed, как в `target-profiles`).
  - **Guardrail (`--hard`):** под `mode=chore` ослабить до «нужен `CHORE-PLAN.md` + chore-gate маркер» вместо
    `gate1.approved` + `plan-review.md`. Модели: судит триаж (opus), остальная полоса — cheap tier.
  - _Развилка оператора:_ мини-гейт до пуша (рекоменд.) **или** гейт = сам CI (зелёный PR и есть приёмка).
  _Зависимость:_ последнюю милю (git/PR/наблюдение CI) закрывает пункт ниже.
- [x] **🔴 Правило старта работы: pull транка → ветка по git-conventions → работа (git — ничей).** ✅ Провязано
  (#58, роль `@git-hand`/Torvalds). Реализовано: роль-порт (`_shared/git-hand.md`, режимы `start`/`terminal`, 4
  абстрактных глагола); реестр провайдеров `harness/vcs-providers.json` (`gh` живой + `mcp-github`/`mcp-bitbucket`/
  `local` заглушки, open-closed как `target-profiles.json`); izi wired (WORKING-BRANCH перед имплементером +
  TERMINAL между `@fagan accepted` и Gate #2); on-trunk poka-yoke в обоих гардрэйлах (`gate-check.mjs`,
  `rational-guardrail.ts`) со смоук-паритетом (claude 25/25 + opencode 35/35). _Остаток — только live-демо на
  chore-полосе (клауза DoD «демо на chore»): провязка есть, живого прогона «харнес сам довёл до зелёного PR» пока нет._
  `@hughes`
  по определению «NO git — no branches/commits/PR» (`izi.md`), и НИ ОДНА роль не владеет git-жизненным циклом.
  Итог: конвейер оставляет артефакт в рабочем дереве, а pull транка / отведение ветки / commit / push / PR
  оператор делает руками. При этом нет гарантии, что работа стартует **от свежего транка** (риск расхождения с
  `main`) и что ветка заведена **по `git-conventions`** (TBD: одна задача = одна ветка = один PR); дисциплина уже
  написана в скилле `git-conventions`, но **не провязана ни в один агент**. Фикс:
  - **Механический шаг «WORKING-BRANCH» в izi перед IMPLEMENTATION** — делегирует роль с git-под-скиллом
    `git-conventions`: `git fetch && pull` транка → `checkout -b <type>/<slug>` → статус-строка `on <branch>
    from <sha>` (izi только читает, ничего не вычисляет). Стартуем от свежего транка, не от устаревшего дерева.
  - **Старт на транке — блокируется** (STOP/guardrail), а не молча допускается (poka-yoke, как фронтдор).
  - **Терминальный git-шаг приёмки — ставится между `@fagan accepted` и Gate #2.** `@fagan accepted` = граница
    «работа завершена И провалидирована» (локальный DoD green, `@wip` снят) → коммитим ТОЛЬКО провалидированное
    состояние, не раньше. В TBD Gate #2 = решение о мёрже, значит PR+зелёный CI обязаны материализоваться ДО
    гейта. Роль (напр. `@git-hand`, cheap tier) на `accepted`: `commit` (сообщение по `git-conventions`) → `push`.
  - **Два этапа, жёстко упорядоченные — CI нельзя проверить до пуша** (гоняется удалённо на запушенном коде):
    - **Этап 1 — локальная валидация** = существующий `@fagan` (build/test/DoD green, `@wip` снят). Гейтит commit.
    - **commit → push** (по git-conventions) — только на зелёном Этапе 1.
    - **Этап 2 — удалённая CI-верификация** = open/update PR → watch CI до green. Это ВТОРАЯ независимая валидация
      на нейтральной площадке (чистый раннер, без локальных кэшей) — ловит класс «toolchain-скос / works-on-my-machine»
      (тот самый эпизод «fagan чинит докер»). Красный CI → `@linger` fix → re-push → re-watch (предохранитель K).
      Зелёный → презентовать Gate #2 с доказательством (зелёный PR). Для chore-полосы «команда верификации» из
      `CHORE-PLAN.md` И ЕСТЬ Этап 2 → chore и полный пайплайн делят один терминальный git+CI-шаг, без дублирования.
  - **Конфигурируемость по среде — провайдер-адаптер** (open-closed, как `target-profiles.json`). Провайдеро-зависимы
    ровно две операции: (1) push/PR — как предлагается изменение; (2) «CI зелёный?» — как читается сигнал. izi
    механически читает `provider` из конфига среды → делегирует git-роли → **роль грузит нужный скилл**:
    - `provider=github` → `gh`/GitHub API (io-скилл `git-github`; `gh run watch`/checks API);
    - `provider=gitlab|bitbucket|…` → **MCP-скилл** форжа (push/PR + pipeline-API через MCP);
    - `provider=local` → только commit, без PR; Этап 2 пропущен (мёрж локально).
    Харнес декларирует ЧТО нужно (предложенное изменение + сигнал «зелёно»), провайдер даёт КАК. Новая среда =
    один provider-entry + один скилл, ноль правок стадий. `git-conventions` (сообщение коммита) — общий, провайдер
...
- [ ] **T4 · P1 — ПЕРИОДИЧЕСКИЙ авто-апдейт** (главное требование — «main продвинулся → подтянуть», flutter/omz-стиль):
  ...
  - **Безопасность:** `--ff-only`; offline/`git fetch` fail → тихо пропустить (offline-tolerant); **НИКОГДА
    не тянуть mid-session** (только на старте — иначе симлинкнутые скиллы/роли меняются под работающим агентом);
    при нескольких активных сессиях с одним клоном — lock/pull-только-на-старте, чтобы не менять файлы под соседом.
