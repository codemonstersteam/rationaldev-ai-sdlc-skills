...
**Текущий статус (честно):** в отладке одна ячейка — `задача=фича/модуль · приложение=go-api ·
среды=CI→PROD-канарейка`. Остальные оси (web/mobile/cli, epic/hotfix/рефакторинг) — вектор, ещё не
реализованы. Граф детерминированного конвейера ролей — **[`docs/concept-api-workflow.md`](docs/concept-api-workflow.md)**.
