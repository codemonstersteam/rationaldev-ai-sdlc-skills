...
// (качество прозы) не трогает — она остаётся @fagan. Профиль-aware: для cli пропускает A4 HTTP-таблицу,
// но A5 pipe-описание требует ВСЕГДА (у CLI pipe = data-flow команды/head-трубы «как работает и где ломается»).
...
// --- A4 — таблица API/команд (service: method|resource; cli: command|args) ---
if (!isCli && !/\|\s*(method|метод)\s*\|/i.test(README)) errors.push("A4: нет API-таблицы method|resource|action")

// --- A5 — pipe-описание (поток данных по шагам через `|` в code-блоке) — для ЛЮБОЙ программы, вкл. CLI ---
// (CLI не освобождён: pipe = data-flow команды/head-трубы «как работает и где ломается»).
const fences = README.split("```")
const hasPipe = fences.some((seg, i) => i % 2 === 1 && /(^|\n)\s*\|\s*\S/.test(seg))
if (!hasPipe) errors.push("A5: нет pipe-описания (поток данных по шагам через `|` в code-блоке; для CLI — data-flow команды/head-трубы)")
