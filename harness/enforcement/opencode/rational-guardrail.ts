...
export const RationalGuardrail: Plugin = async ({ directory, worktree, client }: any) => {
  ...
  // ПРИНЦИП: enforcement-плагин НЕ делает сетевой/блокирующий I/O на init (иначе изолированная сеть вешает
  // старт opencode — найдено live). Self-update (T4) вынесен из плагина в shell-профиль (`rationaldev autocheck`
  // в фоне, как omz). Здесь — только чистая логика гейтов.
