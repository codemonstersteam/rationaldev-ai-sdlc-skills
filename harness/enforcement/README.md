...
1. **Gate #1.** Делегирование роли `implementer` запрещено, пока нет
   ...
   и только когда план собран. Пункт меню — отдельный канал: opencode ловит событие `question.replied`,
   claude — `PostToolUse[AskUserQuestion]` (выбор опции промпта не порождает, поэтому один и тот же
   `gate-approve.mjs` подписан на два события). Читается ТОЛЬКО выбор оператора, не текст вопроса. Агенту запись маркера (`touch`/`>`/write/edit) заблокирована.
2. **Gate #2 (мерж).** Делегирование `@ledger` (закрытие прогона: тег → заметка `refs/notes/ledger` →
