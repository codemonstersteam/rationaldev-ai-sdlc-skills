...
// Через loadModelsConfig — учитывает RATIONALDEV_MODELS override (иначе CLI показывал бы клон-дефолт
// висячим даже после того, как проекции перегенерированы с override → ложный fail).
...
// машинные строки для install.sh (TIERS — из override-merged конфига, не клон-дефолт)
const t = (models[runner] || {}).tiers || {}
console.log(`TIERS: large=${t.large || "(наследует)"} small=${t.small || "(наследует)"}`)
