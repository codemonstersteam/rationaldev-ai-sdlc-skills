...
[ "$(ls "$P/.claude/agents"/*.md 2>/dev/null | wc -l | tr -d ' ')" = 17 ] || fail "claude: ролей не 17"; ok
...
[ "$(ls "$P/.opencode/agent"/*.md 2>/dev/null | wc -l | tr -d ' ')" = 17 ] || fail "opencode: агентов не 17"; ok
...
[ "$(ls "$P/.agents/roles"/*.md 2>/dev/null | wc -l | tr -d ' ')" = 17 ] || fail "codex: ролей не 17"; ok
