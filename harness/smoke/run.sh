...
[ "$(ls "$P/.claude/agents"/*.md 2>/dev/null | wc -l | tr -d ' ')" = 13 ] || fail "claude: ролей не 13"; ok
...
[ "$(ls "$P/.opencode/agent"/*.md 2>/dev/null | wc -l | tr -d ' ')" = 13 ] || fail "opencode: агентов не 13"; ok
...
[ "$(ls "$P/.agents/roles"/*.md 2>/dev/null | wc -l | tr -d ' ')" = 13 ] || fail "codex: ролей не 13"; ok
