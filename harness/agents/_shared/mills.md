...
- **заголовки тикетов машиночитаемы** — запусти `node harness/validate-tickets.mjs` (детерминированно):
  у каждого тикета есть `type`/`blocked_by`/`inputs` (+`io` для module), `blocked_by` ведёт на существующий
  тикет, `inputs`-пути есть, scaffold ровно один. Ненулевой exit = **blocker** (izi иначе не сроутит механически).
