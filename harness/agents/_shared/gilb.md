...
temperature: 0.1
steps: 15
description: "Stage 0: front-door requirements. Raw business requirement (BR) → measurable BRD (data-dictionary per field with fit criteria, failure-map, open-questions) ready for the pipeline. Never invents field ranges — unresolved → question to operator. Keywords: requirements, BRD, measurability, fit criterion, intake."
skills: [requirements-intake]
inputs: [TASK.md, .agent/planner/brd.md]
outputs: [.agent/planner/brd.md]
permission:
  read: allow
  grep: allow
  glob: allow
