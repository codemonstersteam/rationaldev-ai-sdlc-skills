...
temperature: 0.2
steps: 25
description: "Component-test implementer: cases already designed per Cockburn — mechanically lays them into executable tests, drives to RED. STRICT lane (component-tests skill): .feature + step-defs + stubs + @wip. FOREIGN lane (mode=foreign, conform-tests skill): the repo's NATIVE runner (JUnit/pytest/…) per the @surveyor map, native fixtures, no Gherkin/Docker. Invents no new scenarios. Call on a component ticket. Keywords: component tests, conform tests, RED, @wip, gherkin, native runner, foreign, fixtures, steps, stubs."
skills: [component-tests, conform-tests]
...
permission:
  ...
  edit:
    ...
    # foreign lane (mode=foreign): native test dirs of a non-harness repo (dormant on strict tickets — services use component-tests/)
    "src/test/**": allow
    "test/**": allow
    "tests/**": allow
    "spec/**": allow
