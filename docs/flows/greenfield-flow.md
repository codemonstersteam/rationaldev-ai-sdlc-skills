...
- **Досемверной ветви в обход Gate #1 нет.** Правка существующего кода в 1 модуль без смены контракта —
  это `patch` (см. [`patch-flow.md`](./patch-flow.md)); новый код в 1 модуль — **вырожденный greenfield-`modular`**
  (планируется как обычный modular, а не в обход `@hughes`). `level=epic` (>2 модулей или >1 сервиса) —
  **STOP**: алгоритм эпика не реализован.
