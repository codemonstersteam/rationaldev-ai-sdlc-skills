...
`frontmatter` = машиночитаемый конфиг для **харнеса** (`tier` — абстракция размера роли,
mode, temperature, steps, `skills:`, `inputs/outputs`, `permission:`, `description` для
роутинга); **тело** = system-prompt для **модели** (поведение, STOP-правила, гейты).
Разделение даёт один источник → проекции под все раннеры + машинную валидацию конфига.
См. [roles.md](roles.md).

### Конфигурация моделей (`harness/models.config.json`)
Конкретные имена моделей вынесены из ролей в `harness/models.config.json` (харнес не привязан
к провайдеру). На раннер: `tiers` (дефолт по `tier` роли) + `roles` (оверрайд роли, важнее
тира); резолвинг `roles > tiers > пусто` (пусто → роль наследует модель пользователя).
`install.sh`/`install.ps1` при установке **интерактивно** спрашивают большую/среднюю/малую
модель (флаг `--no-input` пропускает диалог). Механика и сценарии — [`harness/README.md`](../../harness/README.md).
