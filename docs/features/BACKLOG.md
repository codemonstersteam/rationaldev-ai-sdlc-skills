...
| — | **route-foreign-lane** (4-й вес конвейера для ЧУЖИХ репо: provenance-ось, conform-не-impose, репо-локальная `docs/design/_harness/`) | 🔴 P0 | ✅ **реализован** (T1–T8: triage→surveyor→change-intake→ticketer(cheat-sheet)→planner→mills-light→gate1→conform-tests/hughes-rework→fagan(verification-cmd); enforcement isForeignMode; строгие лейны нетронуты); хвост — live-прогон | [route-foreign-lane.md](./route-foreign-lane.md) |
| — | **foreign-designer-lane** (дизайн модулей ченджа для foreign: новая роль `@foreign-designer`/Parnas — module-tree+contracts+C4+ADR в conform, чтобы фичу проектировать качественно) | 🔴 P0 | ✅ **реализован** (F1–F5: роль @foreign-designer + врезка izi 2.5 по design=needed; change-intake сигнал; ticketer из дерева; mills ревью consequent⊆antecedent/C4/ADR; строгий moduledesigner нетронут); хвост — live | [foreign-designer-lane.md](./foreign-designer-lane.md) |
