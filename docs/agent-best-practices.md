...
| 13 | **Промпты ролей/агентов — по-английски** (для англо-центричных GLM/Qwen/Claude): тот же смысл дешевле в токенах. | **Замерено:** тела 14 ролей RU→EN = **−38% токенов при том же объёме** (кириллица 1.63× ток/символ). Роль резидентна в промпте каждого хода → прямая экономия latency+стоимости. [`../../experiments/token-bench/role-language-token-cost.md`](../../experiments/token-bench/role-language-token-cost.md). |
