...
**Применяется на:** стадия PLANNING (Planner), PLAN REVIEW (Plan Reviewer).
**Цель:** агент понимает архитектуру системы и смежных систем, проектирует изменение так, чтобы оно вписывалось в существующий ландшафт и корректно документировало сетевую связанность для безопасности.
...
Если карты связанности или контрактов нет — это сигнал: задай уточняющий вопрос человеку (Discovery ведёт инженер + ИИ), не выдумывай топологию.
...
- service-orders (изменение)
- service-notifications (новый consumer события)

### Взаимодействия
| От | К | Тип | Протокол | Направление |
|----|---|-----|----------|-------------|
| service-orders | kafka:orders.completed | async | Kafka | publish |
| service-notifications | kafka:orders.completed | async | Kafka | subscribe |
...
- [x] Idempotency обеспечена ключом order_id
