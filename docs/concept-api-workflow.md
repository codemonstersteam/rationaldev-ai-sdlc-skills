...
### Как передаются скиллы (progressive disclosure)

Скилл — это процедура (не справочник), но **само тело скилла в промпт не встраивается**. Контекст
держит только `name` + `description`; тело подгружается **по требованию** нативным тулом `skill` и
остаётся в истории как `tool_result` (path B, задача #40). Три слоя:

| Слой | Что | Как подцепляется |
|---|---|---|
| **База (always-on)** | `program-implementation`, `code-style`, `communication`, `memory` | ядро роли — не перечисляется в тикете |
| **io add-on** | `http-io` · `db-io`+`db-schema` · `queue-io` · `llm-client` | **детерминированно** io-router'ом по полю `io:` модуля; едет в `skills:` тикета |
| **core + companion** | большой SKILL.md разбит на ядро (≤300 строк) + companion-файлы | companion читается, только когда реально нужен (например `service-scaffold/planner.md` — только у планировщика) |

Итог: в окне модели — минимальный высокосигнальный набор, а не весь каталог скиллов. Встраивание
тел целиком (то, что отрасль отвергла) на топ-скиллах давало ~10 % входного контекста «шумом» —
progressive disclosure это убирает (замер #40: −34.6 %).

### Лучшие практики — что берём и почему

Флоу собран по первичным инженерным публикациям (Anthropic / OpenAI / Cognition / HumanLayer);
полный список с привязкой — [`docs/agent-best-practices.md`](agent-best-practices.md). Ключевое:

| Практика | Почему | Где в харнесе |
|---|---|---|
| **Контекст-инжиниринг — главная дисциплина**: наименьший высокосигнальный набор токенов; борьба с «context rot» и «dumb zone» (провал recall в середине большого окна) | длинный контекст **снижает** качество, а не только дорожает | тикет минимального контекста; per-slice durable артефакты вместо одного мега-промпта |
| **Progressive disclosure скиллов** (Anthropic Agent Skills: discovery → activation → execution) | тело нужно только в момент исполнения — до этого оно шум | `skill`-как-тул, ядро+companion, правило «SKILL ≤300 строк» |
| **Инструментам — столько же внимания, сколько промптам**; токен-экономный вывод (ID/срезы, не дампы) | тул-вывод раздувает контекст сильнее промпта | izi проверяет артефакты по точному пути (`read`/`ls`, не `glob`-дамп); роли-исполнители не сканируют дерево |
| **Один агент по умолчанию; мульти — под форму задачи, с изолированными субагентами**; «делись полными трейсами, не сообщениями» (Cognition) | наивное дробление контекста теряет нюанс и рассинхронизирует агентов | flat `izi` depth-1; риск потери нюанса лечим **замороженным контрактом** + тикетами с явными `inputs` |
| **Детерминированные гейты + trajectory-eval** (LangChain: путь важнее финального ответа) | LLM-судья без якоря нестабилен; путь (ретраи, тул-вызовы) диагностирует лучше | валидаторы `validate-*` + eval-слой (`eval-run`/`eval-judge`, см. ниже) |
