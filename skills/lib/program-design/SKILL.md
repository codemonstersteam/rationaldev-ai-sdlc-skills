...
если потерялся — вернись сюда, найди текущий шаг, открой его якорь «Вход/Выход».
...
**Шаблон тикета и заполненный пример вынесены в [`reference/ticket-template.md`](reference/ticket-template.md)** (открывается на этом шаге, не держится в контексте постоянно).

**Действие (обязательно):** открой `reference/ticket-template.md`, скопируй нужный шаблон в `backlog.md` дословно, затем замени каждый плейсхолдер `<…>` конкретикой по таблице подстановок выше. Не воспроизводи шаблон по памяти — копируй из файла.
...
исполнитель распознаёт «пакет принят» в main (см. `skills/program-implementation/SKILL.md`
